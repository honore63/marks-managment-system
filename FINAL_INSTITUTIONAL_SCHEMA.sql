...
CREATE TABLE IF NOT EXISTS schools (
    sdms_code TEXT PRIMARY KEY, -- 6-digit SDMS school code (e.g., 541021)
    name TEXT NOT NULL,
    district TEXT,
    sector TEXT,
...
CREATE TABLE IF NOT EXISTS school_settings (
    school_code TEXT PRIMARY KEY REFERENCES schools(sdms_code) ON DELETE CASCADE,
...
CREATE POLICY school_isolation_profiles ON profiles FOR ALL USING (school_code = (SELECT school_code FROM profiles WHERE id = auth.uid()) OR role = 'system_admin');
CREATE POLICY school_isolation_students ON students FOR ALL USING (school_code = (SELECT school_code FROM profiles WHERE id = auth.uid()) OR role = 'system_admin');
CREATE POLICY school_isolation_marks ON marks FOR ALL USING (school_code = (SELECT school_code FROM profiles WHERE id = auth.uid()) OR role = 'system_admin');
CREATE POLICY school_isolation_classes ON classes FOR ALL USING (school_code = (SELECT school_code FROM profiles WHERE id = auth.uid()) OR role = 'system_admin');
CREATE POLICY school_isolation_subjects ON subjects FOR ALL USING (school_code = (SELECT school_code FROM profiles WHERE id = auth.uid()) OR role = 'system_admin');
CREATE POLICY school_isolation_assignments ON teacher_assignments FOR ALL USING (school_code = (SELECT school_code FROM profiles WHERE id = auth.uid()) OR role = 'system_admin');
CREATE POLICY school_isolation_settings ON school_settings FOR ALL USING (school_code = (SELECT school_code FROM profiles WHERE id = auth.uid()) OR role = 'system_admin');
