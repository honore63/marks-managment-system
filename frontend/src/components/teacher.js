...
async function initTeacherPortal() {
  try {
    ...
    // Ensure Dashboard (My Classes) and Verification (Assigned Subjects) are always visible for any teacher
    ...
    if (sidebarUserRole) {
        ...
        else sidebarUserRole.textContent = 'Pedagogical Teacher';
    }
    ...
    if (typeof SYNC !== 'undefined' && SYNC.start) {
        await SYNC.start();
        console.log('[SYNC] Teacher Node Online');
    }
...
document.addEventListener('click', (e) => {
    if (window.innerWidth <= 1024) {
        if (e.target.classList.contains('nav-link') || e.target.closest('.nav-link') || e.target.classList.contains('dropdown-item') || e.target.closest('.dropdown-item')) {
            const navMenu = document.querySelector('.nav-menu');
...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Initialize Professional Signs (Lucide)
    if (window.lucide) {
        lucide.createIcons();
    }
});
...
async function switchView(viewId, el) {
    ...
    const META = {
        ...
        'profile':          ['Teacher Profile',                'Account → Settings'],
    };
...
async function renderProfile() {
    ...
    if (MY_PROFILE) {
        const fullName = MY_PROFILE.full_name || 'Teacher';
        ...
        if (sidebarRole) {
            ...
            sidebarRole.textContent = roles.length > 0 ? roles.join(' & ') + ' TEACHER' : 'PEDAGOGICAL TEACHER';
        }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    console.log('[MMS] Teacher Node fully synchronized.');
});
...
async function generateExcelTemplateForSubject(sid, sName, cid, cName) {
    ...
    try {
        ...
        console.log('[INIT] Teacher Workspace fully operational.');
    } catch (err) {
