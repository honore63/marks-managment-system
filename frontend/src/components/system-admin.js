...
async function initSystemAdmin() {
    try {
        ...
        if (typeof SYNC !== 'undefined' && SYNC.start) {
            ...
            SYNC.on('teachers', () => {
                console.log('[SYNC] Global Teacher Membership Updated');
                updateStats();
...
async function updateStats() {
    try {
        const [schools, teachers, students, marks] = await Promise.all([
            _supabase.from('schools').select('*', { count: 'exact', head: true }),
            _supabase.from('profiles').select('id', { count: 'exact', head: true }).in('role', ['admin', 'teacher']),
            _supabase.from('profiles').select('id', { count: 'exact', head: true }).eq('role', 'student'),
        ...
        if (el('stat-total-admins')) el('stat-total-admins').textContent = teachers.count || 0;
        if (el('stat-total-students')) el('stat-total-students').textContent = students.count || 0;
...
async function handleRegisterSchool(event) {
    ...
    try {
        ...
        // 3. Provision Institutional Account via Automated RPC
        // Rule: Default Email: sdms[CODE]@mms.rw | Default Password: Admin@[CODE]
        const { data: rpcData, error: rpcError } = await _supabase.rpc('create_school_admin', {
            p_school_code: sdmsCode,
            p_full_name: adminName
        });

        if (rpcError) throw rpcError;

        const adminId = rpcData[0].id;
        const sdmsEmail = rpcData[0].email;
        const initialPassword = `Admin@${sdmsCode}`;

        // 4. Create School Node (Metadata)
        ...
        toast(`âœ… Institutional Node ${sdmsCode} provisioned and active!`, 'success');
...
async function handleLogout() {
    if (confirm('Terminate System Administration Session?')) {
        try {
            if (typeof window.SYNC !== 'undefined' && window.SYNC.stop) window.SYNC.stop();
