...
async function initNotificationSystem(schoolCode, userId) {
    try {
        ...
        // Real-time listener
        const activeChannels = _supabase.getChannels();
        if (!activeChannels.some(c => c.topic === 'realtime:mms-hq-notifications')) {
  ...
  if (SYNC && typeof SYNC.on === 'function') {
    ...
    const refreshCurrentView = async () => {
        ...
        if (activeView === 'view-dashboard') {
            if (typeof renderTeachersMonitor === 'function') await renderTeachersMonitor();
    ...
    SYNC.on('teachers', (payload) => {
      console.log('[UPDATE] Teacher registry changed:', payload.eventType);
...
async function switchView(viewId, triggerEl) {
    ...
    const headers = {
      ...
      'teachers':       ['Teachers Directory',   'Registry', 'Pedagogical Teachers'],
    ...
    try {
        if (viewId === 'dashboard')        { await renderDashboard(); await renderTeachersMonitor(); }
        else if (viewId === 'management')  await renderSetup();
        else if (viewId === 'students')    await renderCohortRegistry();
        else if (viewId === 'teachers')    await renderTeachersRegistry();
...
async function renderTeachersMonitor() {
    const tbody = el('teachers-activity-tbody');
    if (!tbody) return;
    ...
    try {
        ...
        if (teacherData.length === 0) {
            tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding: 3rem; color: #94a3b8;">No teachers registered in this node.</td></tr>`;
            return;
        }
        ...
        console.error('[TEACHER] Monitor failed:', err);
    }
}
...
async function renderTeachersRegistry() {
    ...
    try {
        tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; padding: 2rem;">
            <div class="loader-linear"></div><br>Synchronizing Teacher Registry...
        </td></tr>`;
        ...
        if (teachers.length === 0) {
            tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; padding: 3rem; color: #94a3b8;">No teachers registered. Click 'Register New Teacher' to begin.</td></tr>`;
            return;
        }
...
async function deleteTeacher(id, name) {
    if (!confirm(`⚠️ PERMANENT ACTION: Are you sure you want to remove ${name} from the institutional teacher registry? This will also wipe their teaching assignments.`)) return;

    toast('🚮 Terminating teacher node...', 'info');
    try {
        ...
        container.innerHTML = classes.map(c => {
            ...
            return `
                <div class="matrix-card" style="cursor: pointer; transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); border: 2px solid transparent;" 
                    ...
                    <button class="btn btn-primary" style="width:100%; margin-top:8px; padding: 0.2rem; font-size:0.45rem; background: #1e1b4b; border-color: #1e1b4b; white-space: nowrap;">
                        PREVIEW & DOWNLOAD
                    </button>
                </div>
            `;
...
async function addTeacher() {
    ...
    if (!fname || !lname || !email || !sdms) {
        return toast('Please fill in all mandatory teacher fields.', 'warning');
    }
    ...
    toast('Provisioning teacher account...', 'info');
    if (btn) {
        btn.disabled = true;
...
async function processRejection(markId) {
    ...
    toast('Record sent back to teacher node.', 'warning');
    await renderMonitoringQueue();
    await updateInstitutionalStats();
}
...
async function saveInstitutionalSettings() {
    ...
    if (error) {
        ...
        SCHOOL_INFO = { ...SCHOOL_INFO, ...updates };
        toast('Academic Node Synced. Term updated for all teachers.', 'success');
        await syncInstitutionalNodes();
        await updateInstitutionalStats();
 ...
 */
async function renderTeachersMonitor() {
    const tbody = document.getElementById('teachers-activity-tbody');
    if (!tbody) return;
    ...
    try {
        ...
        tbody.innerHTML = html || '<tr><td colspan="5" style="text-align:center; padding: 3rem; color: #94a3b8;">No teacher assignments found in registry.</td></tr>';
    } catch (e) {
...
async function openEditAssignments(teacherId) {
    const f = (window._cachedTeachers || []).find(t => t.id === teacherId);
    if (!f) return;
...
async function saveAssignmentCorrections() {
    ...
    toast('Teacher Profile and Jurisdictions successfully updated.', 'success');
...
async function processAddTeacher() {
    try {
        ...
        // ── STEP 1 & 2: Provision Institutional Account via Automated RPC ──
        // This handles rule-based email generation and enables JIT auth provisioning
        const { data: rpcData, error: rpcError } = await _supabase.rpc('create_teacher', {
            p_school_code: schoolCode,
            p_full_name: `${fname} ${lname}`,
            p_sdms_code: sdms
        });

        if (rpcError) throw rpcError;
        
        const teacherId = rpcData[0].id;
        const generatedEmail = rpcData[0].email;
        ...
        try {
            ...
            await sendTeacherOnboardingInvite({
                teacherId: teacherId,
 ...
 * Automatically dispatches professional Email/SMS to newly registered teachers.
 */
async function sendTeacherOnboardingInvite(data) {
    if (!data.email) {
...
async function dispatchTwilioSMS(to, body) {
    ...
    try {
        ...
        toast('SMS notification delivered to teacher phone.', 'success');
...
async function renderTeachersRegistry() {
    const [teachers, allAssignments, classes, subjects] = await Promise.all([
        DB.getTeachers(), DB.getTeacherAssignments(), DB.getClasses(), DB.getSubjects()
    ]);
    window._cachedTeachers = teachers;
...
function renderTeachersList(teachers) {
    ...
    if (!tbody) return;
    tbody.innerHTML = teachers.map(f => {
        ...
        <tr>
            ...
            <td style="text-align: right; display: flex; gap: 0.5rem; justify-content: flex-end;">
               ...
               <button class="btn" style="padding: 4px 8px; font-size: 0.65rem; color: var(--danger); border-color: #fecaca; background: #fef2f2;" title="Terminate Teacher Account" onclick="handleDeleteTeacher('${f.id}', '${f.full_name}')"><i data-lucide="trash-2" style="width:14px; height:14px;"></i></button>
            </td>
        </tr>`;
    }).join('') || '<tr><td colspan="5" style="text-align:center; padding:3rem; color:#94a3b8;">No teachers.</td></tr>';
}
...
window.generateAllReportsPdf = async function(targetClassId = 'all', targetSubs = [], targetAsses = []) {
    ...
    const selSubIds = targetSubs.map(s => String(s.id)).filter(id => !id.startsWith('std_'));
    const selAssIds = targetAsses.map(a => String(a.id)).filter(id => !id.startsWith('std_'));

    // Merge DB subjects with Virtual (std_) subjects
    const subjects = targetSubs.map(sel => {
        const dbMatch = allDBSubjects.find(s => String(s.id) === String(sel.id));
    ...
    const filteredMarks = allMarks.filter(m => {
        const isClassMatch = targetClassId === 'all' || String(m.class_id) === String(targetClassId);
        const isSubMatch = selSubIds.length === 0 || selSubIds.includes(String(m.subject_id));
        const isAssMatch = selAssIds.length === 0 || selAssIds.includes(String(m.assessment_id));
        return m.is_approved && isClassMatch && isSubMatch && isAssMatch;
    });
    ...
    const targetStudents = targetClassId === 'all' 
        ? students 
        : students.filter(s => String(s.class_id) === String(targetClassId));
    ...
    for (const student of targetStudents) {
        ...
        const stClass = classes.find(c => String(c.id) === String(student.class_id))?.name || 'GENERAL';
        const stMarks = filteredMarks.filter(m => String(m.student_id) === String(student.id));
        ...
        studentSubjects.forEach(sub => {
            const sm = stMarks.filter(m => String(m.subject_id) === String(sub.id));
...
async function handleLogout() {
    if (confirm('Disconnect from institutional node? All unsaved active sessions will be terminated.')) {
        try {
            ...
            if (typeof DB !== 'undefined' && DB.signOut) await DB.signOut();
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    SYNC.on('assignments', async () => {
        ...
        if (active === 'view-dashboard') await renderTeachersMonitor();
        if (active === 'view-teachers') await renderTeachersRegistry();
    });
    SYNC.on('marks', async () => {
        await updateInstitutionalStats();
        const active = document.querySelector('.view.active')?.id;
    ...
    SYNC.on('teachers', async () => { 
        await updateInstitutionalStats(); 
        if (document.querySelector('.view.active')?.id === 'view-teachers') await renderTeachersRegistry(); 
    });
...
window.exportClassProclamation = async function(classId, targetSubs = [], targetAsses = []) {
    ...
    try {
        ...
        const assignment = assignments.find(a => String(a.class_id) === String(classId) && a.type === 'class');
        let classTeacherName = "DEPARTMENT HEAD";
        if (assignment) {
            const tr = teachers.find(t => String(t.id) === String(assignment.teacher_id));
            if (tr) classTeacherName = tr.full_name.toUpperCase();
        }

        const classStudents = stds.filter(s => String(s.class_id) === String(classId));
        ...
        const targetSubsStr = targetSubs.map(String);
        const targetAssesStr = targetAsses.map(String);
        
        const classMarks = marks.filter(m => 
            String(m.class_id) === String(classId) && 
            m.is_approved &&
            String(m.term) === activeTerm &&
            (targetSubsStr.length === 0 || targetSubsStr.includes(String(m.subject_id))) &&
            (targetAssesStr.length === 0 || targetAssesStr.includes(String(m.assessment_id)))
        );
        
        const classSubjects = subs.filter(s => targetSubsStr.length === 0 || targetSubsStr.includes(String(s.id)));
...
window.generatePassRateReport = async function() {
    ...
    try {
        ...
        if (targetClassId === 'all') {
            ...
            targetClasses = classes.filter(c => String(c.id) === String(targetClassId));
        }
        ...
        for (const cls of targetClasses) {
            const classStudents = students.filter(s => String(s.class_id) === String(cls.id));
            const classMarks = allMarks.filter(m => String(m.class_id) === String(cls.id) && String(m.term) === String(term) && m.is_approved);
...
window.generateSubjectSuccessReport = async function() {
    ...
    try {
        ...
        const filteredMarks = allMarks.filter(m => 
            ...
            m.is_approved && 
            (subjectId === 'all' || String(m.subject_id) === String(subjectId)) &&
            (classId === 'all' || String(m.class_id) === String(classId))
        );

        const targetClasses = classId === 'all' ? classes : classes.filter(c => String(c.id) === String(classId));
        const reportData = [];

        targetClasses.forEach(cls => {
            const clsStudents = students.filter(s => String(s.class_id) === String(cls.id));
            const clsMarks = filteredMarks.filter(m => String(m.class_id) === String(cls.id));
...
window.generateGradeDistributionReport = async function() {
    ...
    try {
        ...
        const classMarks = allMarks.filter(m => String(m.class_id) === String(classId) && String(m.term) === String(term) && m.is_approved);
...
window.executeReportCardGeneration = async function() {
    ...
    try {
        ...
        const subjects = allSubjects.filter(s => selSubIds.includes(String(s.id)));
        const activePillars = allAssessments.filter(a => selAssessIds.includes(String(a.id))).map(a => ({
        ...
        if (classId !== 'all') {
            targetStudents = allStudents.filter(s => String(s.class_id) === String(classId));
        }
